function [qamSig,deSig] = qamModu(bits,modFormat)
% Description:
%     QAMMODU Generate qam signal
%
% EXAMPLE:
%     [qamSig,bits] = qamModu(bits,modFormat)
%
% INPUT:
...
%
%  Copyright, 2016, H.B. Zhang, <hongbo.zhang83@gmail.com>
%
% Modifications:
...
bitInputLen    = length(bits);

nBpB           = format2nBpB(modFormat);
symsLen        = floor(bitInputLen/nBpB);
bitLen         = nBpB*symsLen;
bits           = bits(1:bitLen);

modFormat = upper(modFormat);
if strcmpi(modFormat,'EDB')
    qamSig     = duobinary(bits);  % duobinary(bits,'modified')
    deSig      = [];
else
    switch modFormat
        case {'BPSK','NRZ'}
    ...
    bitsMatrix   = vec2mat(bits,nBpB);
    deSig        = bi2de(bitsMatrix,'left-msb'); % {0,1,2,3}
    mapIdx       = deSig+1;
    qamSig       = cnstPattern(mapIdx);
end
end